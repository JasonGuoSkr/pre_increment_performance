...
# 参数
inputPath = "E:/中泰证券/策略/潜伏业绩预增策略/权重调整/"
outputPath = "E:/中泰证券/策略/潜伏业绩预增策略/权重调整/"


# 数据导入
ratio_df = pd.read_csv(inputPath + "策略收益率换手率.csv", index_col=0, engine='python')
equity_df = pd.read_csv(inputPath + "策略动态权益.csv", index_col=0, engine='python')
...
# 数据导出
yearValue_df.to_csv(outputPath + "年度收益分析.csv")
